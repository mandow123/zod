...
try {
  ...
  const discontinuity = migrations.findIndex((name, index) => Number(name.slice(0, 4)) !== index + 1);
  if (discontinuity >= 0) {
    throw new Error(`Migration sequence is not continuous at ${migrations[discontinuity]}.`);
  }
  if (migrations.at(-1) !== '0061_dual_reward_core.sql') {
    throw new Error(`Expected migrations through 0061_dual_reward_core.sql; found ${migrations.length}, latest ${migrations.at(-1) ?? 'none'}.`);
  }
