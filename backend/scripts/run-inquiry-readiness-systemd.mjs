...
const [pairBytes,databaseUrlBytes,auditPepperBytes]=await Promise.all([
  readFile(pairPath,'utf8'),readFile(resolve(credentialDirectory,'kai-probe-database-url'),'utf8'),
  readFile(resolve(credentialDirectory,'kai-probe-audit-pepper'),'utf8'),
]);
const pair=JSON.parse(pairBytes),databaseUrl=databaseUrlBytes.trim(),auditPepper=auditPepperBytes.trim();
if(!pair||Object.keys(pair).sort().join(',')!=='accessToken,idToken,schemaVersion,subjectSha256'
  ...
  ||auditPepper.length<32)throw new Error('KAI_PROBE_EPHEMERAL_INPUT_INVALID');
validateLoopbackProbeDatabaseUrl(databaseUrl);
try{
  const probe=await runInquiryReadinessProbe({accessToken:pair.accessToken,idToken:pair.idToken,
    probeSubjectSha256:pair.subjectSha256,
    probeOrigin:'https://cloudpay.kai.com',environment:{NODE_ENV:'production',MOBILE_API_PROFILE:'inquiry_only',
      PUBLIC_ORIGIN:'https://cloudpay.kai.com',DATABASE_URL:databaseUrl,DATABASE_SSL:'false',AUDIT_PEPPER:auditPepper}});
  process.stdout.write(`${JSON.stringify({ok:probe.ok,refreshPersisted:true,probeRecorded:probe.ok})}\n`);
}finally{await rm(pairPath,{force:true});}
