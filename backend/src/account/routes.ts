...
export async function registerAccountRoutes(
  ...
  app.get('/mobile/v1/legal', async () => {
    if (!config.LEGAL_ENTITY_NAME) throw new AppError('LEGAL_ENTITY_UNAVAILABLE', 503, '运营主体信息尚未配置。');
    return { ok: true, operator: { legalEntityName: config.LEGAL_ENTITY_NAME }, documents: service.legalDocuments(config) };
  });
  ...
  app.get('/mobile/v1/me', async (request) => {
    // Identity bootstrap cannot perform commerce and remains available before
    // legal acceptance so the App can present the mapped local profile.
    const { principal } = await authenticateMobileBootstrapRequest(service, request);
