...
import { planSettlementFee } from '../settlement-fees/engine.js';
...
type ProductRow = QueryResultRow & { id:string; sku:string; title:string; supplier_display_name:string;
  ...
  unit_credit_micros:string; discount_basis_points:number; expected_ship_days:number; specifications:Record<string,unknown> };
type OrderRow = QueryResultRow & { id:string; order_number:string; buyer_subject_id:string; supplier_subject_id:string;
  created_by_user_id:string; product_id:string; status:DeviceOrderStatus; quantity:number; unit_credit_micros:string;
  ...
  logistics_provider:string|null; created_at:Date; updated_at:Date };
...
const productColumns = `id,sku,title,supplier_display_name,supplier_subject_id,activation_status,inventory_total,
  inventory_reserved,inventory_sold,list_price_cny_micros::text,sale_price_cny_micros::text,
  unit_credit_micros::text,discount_basis_points,expected_ship_days,specifications`;
const orderColumns = `id,order_number,buyer_subject_id,supplier_subject_id,created_by_user_id,product_id,status,
  quantity,unit_credit_micros::text,gross_credit_micros::text,service_fee_credit_micros::text,
  supplier_net_credit_micros::text,reservation_transaction_id,resolution_transaction_id,reservation_expires_at,
  confirmed_at,shipped_at,received_at,resolved_at,logistics_provider,created_at,updated_at`;
function product(row:ProductRow):DeviceProduct { return { id:row.id,sku:row.sku,title:row.title,
  ...
  salePriceCnyMicros:BigInt(row.sale_price_cny_micros),unitCreditMicros:BigInt(row.unit_credit_micros),
...
function order(row:OrderRow):DeviceOrder { const optional=(x:string|null)=>x===null?null:BigInt(x); return {
  ...
  createdByUserId:row.created_by_user_id,productId:row.product_id,status:row.status,quantity:row.quantity,
  ...
  resolvedAt:row.resolved_at?new Date(row.resolved_at):null,logisticsProvider:row.logistics_provider,
  createdAt:new Date(row.created_at),updatedAt:new Date(row.updated_at) }; }

const feeTiers:readonly FeeTier[] = [
...
export class PostgresDeviceCommerceStore {
  ...
  async activateProduct(productId:string,supplierSubjectId:string,operatorId:string,now:Date){ return this.database.transaction(async client=>{
    const eligible=await client.query(`SELECT p.subject_id FROM kai_credit_payout_profiles p JOIN trading_subjects s ON s.id=p.subject_id
      WHERE p.subject_id=$1 AND p.status='active' AND s.status='active' FOR UPDATE`,[supplierSubjectId]);
    ...
    const r=await client.query<ProductRow>(`UPDATE physical_device_products SET activation_status='active',supplier_subject_id=$2,
      activated_by_user_id=$3,activated_at=$4 WHERE id=$1 AND activation_status IN ('pending_activation','suspended') RETURNING ${productColumns}`,
    [productId,supplierSubjectId,operatorId,now]); return r.rows[0]?product(r.rows[0]):null; }); }
  async create(input:{id:string;orderNumber:string;buyerSubjectId:string;userId:string;productId:string;quantity:number;
    ...
    return this.database.transaction(async client=>{
      ...
      const r=await client.query<OrderRow>(`INSERT INTO physical_device_orders(id,order_number,buyer_subject_id,supplier_subject_id,
        ...
        VALUES($1,$2,$3,$4,$5,$6,$7,$8,$9,'reserved',$10,$11,$12,$13,$14) RETURNING ${orderColumns}`,
      [input.id,input.orderNumber,input.buyerSubjectId,item.supplierSubjectId,input.userId,item.id,input.clientRequestId,
        input.payloadDigest,input.shippingAddressReference,input.quantity,item.unitCreditMicros.toString(),gross.toString(),tx,input.expiresAt]);
      return {status:'created',order:order(r.rows[0]!)};
    });
  }
  async listOrders(subjectId:string){ const r=await this.database.query<OrderRow>(`SELECT ${orderColumns} FROM physical_device_orders
    WHERE buyer_subject_id=$1 OR supplier_subject_id=$1 ORDER BY created_at DESC,id DESC`,[subjectId]); return r.rows.map(order); }
  ...
  async listAssets(subjectId:string){ const r=await this.database.query<AssetRow>(`SELECT a.id,a.order_id,a.owner_subject_id,a.product_id,
    ...
    WHERE a.owner_subject_id=$1 ORDER BY a.acquired_at DESC,a.id DESC`,[subjectId]); return r.rows.map(row=>({id:row.id,orderId:row.order_id,
  ...
  async action(input:{orderId:string;actorId:string;actorSubjectId:string;side:'buyer'|'supplier';action:'confirm'|'ship'|'receive'|'cancel'|'expire';
    from:DeviceOrderStatus;to:DeviceOrderStatus;clientRequestId:string;payloadDigest:string;now:Date;logisticsProvider?:string;trackingDigest?:string}):Promise<DeviceActionResult>{
    return this.database.transaction(async client=>{
      ...
      const r=await client.query<OrderRow>(`UPDATE physical_device_orders SET status=$2,resolution_transaction_id=$3,
        ...
        tracking_digest=CASE WHEN $2='shipping' THEN $8 ELSE tracking_digest END WHERE id=$1 AND status=$9 RETURNING ${orderColumns}`,
      [current.id,input.to,resolution,fee?.toString()??null,net?.toString()??null,input.now,input.logisticsProvider??null,input.trackingDigest??null,input.from]);
  ...
  async settle(input:{orderId:string;actorId:string;actorSubjectId:string;clientRequestId:string;payloadDigest:string;now:Date}):Promise<DeviceSettlementResult>{
    return this.database.transaction(async client=>{
      ...
      const r=await client.query<{order_id:string;status:'succeeded';gross_credit_micros:string;service_fee_credit_micros:string;net_credit_micros:string;available_at:Date;settlement_transaction_id:string;settled_at:Date}>(`UPDATE physical_device_supplier_settlements SET status='succeeded',settlement_transaction_id=$2,
        settled_by_user_id=$3,settled_at=$4 WHERE order_id=$1 AND status='pending' RETURNING order_id,status,gross_credit_micros::text,
  ...
  private async capture(client:PoolClient,o:DeviceOrder,input:Parameters<PostgresDeviceCommerceStore['action']>[0]){
    ...
    const volume=await client.query<{gross:string}>(`SELECT COALESCE(sum(gross_credit_micros),0)::text gross FROM physical_device_orders
      WHERE supplier_subject_id=$1 AND status='received'`,[o.supplierSubjectId]);
    const plan=planSettlementFee(feeTiers,BigInt(volume.rows[0]?.gross??'0'),o.grossCreditMicros);const tx=randomUUID();
    await client.query(`INSERT INTO kai_credit_transactions(id,idempotency_owner,scope,idempotency_key,payload_digest,reference_type,reference_id,description,status)
