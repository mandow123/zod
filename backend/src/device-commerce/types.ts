...
export type DeviceProduct = Readonly<{
  id: string; sku: string; campaignKey: string; templateKey: string; title: string; supplierDisplayName: string; supplierSubjectId: string | null;
  ...
  listUnitCreditMicros: bigint; unitCreditMicros: bigint; discountBasisPoints: number; expectedShipDays: number;
...
export type DeviceOrder = Readonly<{
  ...
  productId: string; campaignKey: string; campaignVersion: string; status: DeviceOrderStatus; quantity: number;
  unitCreditMicros: bigint; grossCreditMicros: bigint;
  ...
  logisticsProvider: string | null; trackingCiphertext: string | null; createdAt: Date; updatedAt: Date;
}>;
