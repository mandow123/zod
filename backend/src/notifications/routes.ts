...
async function authenticateInstallation(accounts: AccountService, request: FastifyRequest) {
  const authenticated = await authenticateMobileRequest(accounts, request);
  if (!('identity' in authenticated)) {
    throw new AppError('AUTH_DEVICE_CONTEXT_REQUIRED', 503, '推送安装标识尚未完成迁移。');
  }
...
export async function registerNotificationRoutes(
  ...
  app.put('/mobile/v1/devices/push', async (request) => {
    const { principal, identity } = await authenticateInstallation(accounts, request);
