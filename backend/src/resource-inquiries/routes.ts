...
export async function registerResourceInquiryRoutes(app:FastifyInstance,accounts:AccountService,service:ResourceInquiryService){
  ...
  app.post('/mobile/v1/resource-inquiries',async(request,reply)=>{const{principal}=await accounts.authenticate(request.headers.authorization);
  ...
  app.get('/mobile/v1/resource-inquiries',async(request)=>{const{principal}=await accounts.authenticate(request.headers.authorization);
  ...
  app.get('/mobile/v1/resource-inquiries/:inquiryId',async(request)=>{const{principal}=await accounts.authenticate(request.headers.authorization);
  ...
  app.get('/mobile/v1/resource-inquiries/:inquiryId/clarifications',async(request)=>{const{principal}=await accounts.authenticate(request.headers.authorization);
  ...
  app.post('/mobile/v1/resource-inquiries/:inquiryId/clarifications',async(request,reply)=>{const{principal}=await accounts.authenticate(request.headers.authorization);
    ...
    return reply.status(result.replayed?200:201).send({ok:true,...result});});
  app.post('/mobile/v1/resource-inquiries/:inquiryId/cancel',async(request)=>{const{principal}=await accounts.authenticate(request.headers.authorization);
  ...
  app.get('/mobile/v1/provider/resource-inquiries',async(request)=>{const{principal}=await accounts.authenticate(request.headers.authorization);
    return{ok:true,...await service.supplierList(principal,scopedInput(parse(scopedQuery,request.query)))};});
  app.get('/mobile/v1/provider/resource-inquiries/:inquiryId',async(request)=>{const{principal}=await accounts.authenticate(request.headers.authorization);
  ...
  for(const action of ['request-clarification','decline','confirm-capacity'] as const)app.post(`/mobile/v1/provider/resource-inquiries/:inquiryId/${action}`,async(request)=>{
    const{principal}=await accounts.authenticate(request.headers.authorization),params=parse(z.object({inquiryId:uuid}),request.params);
  ...
  app.get('/mobile/v1/operator/resource-inquiries',async(request)=>{const{principal}=await accounts.authenticate(request.headers.authorization);
  ...
  app.get('/mobile/v1/operator/resource-inquiries/:inquiryId',async(request)=>{const{principal}=await accounts.authenticate(request.headers.authorization);
  ...
  app.post('/mobile/v1/operator/resource-inquiries/:inquiryId/assign',async(request)=>{const{principal}=await accounts.authenticate(request.headers.authorization);
