...
const createCommon={startsAt:z.string().datetime({offset:true}),endsAt:z.string().datetime({offset:true}),
  timeZone:z.string().trim().min(1).max(100),confirmBy:z.string().datetime({offset:true}),
  ...
  network:z.enum(['public_internet','private_network','dedicated_line','flexible']),
  storageGiB:z.number().int().positive().max(10000000),dataRegion:z.string().trim().min(2).max(80),
  terms:z.object({termsVersion:z.string().trim().min(1).max(40),privacyVersion:z.string().trim().min(1).max(40),
    inquiryVersion:z.string().trim().min(1).max(40)}).strict()};
const legacyCreate=z.object({candidateId:uuid,gpuCount:z.number().int().positive().max(100000),...createCommon}).strict();
const supplierCreate=z.object({supplierResourceId:z.string().regex(/^(?:gpu|server)-honghuan-[a-z0-9-]{6,100}$/u),
  supplierResourceVersion:z.number().int().positive(),quantity:z.number().int().positive().max(100000),...createCommon}).strict();
...
export async function registerResourceInquiryRoutes(app:FastifyInstance,accounts:AccountService,service:ResourceInquiryService,
  routeProfile:'inquiry_only'|'full_commerce'='full_commerce'){
  if(routeProfile==='full_commerce'){
  ...
  app.post('/mobile/v1/resource-inquiries',async(request,reply)=>{const{principal}=await auth(accounts,request);
    const body=parse(routeProfile==='inquiry_only'?supplierCreate:z.union([legacyCreate,supplierCreate]),request.body);
  ...
  if(routeProfile==='inquiry_only')return;
