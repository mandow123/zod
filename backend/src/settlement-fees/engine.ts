...
import { isCreditCentAligned, KAI_CREDIT_CENT_MICROS } from '../credits/precision.js';
...
export function validateFeeTiers(tiersInput: readonly FeeTier[]) {
  ...
  for (const [index, tier] of tiers.entries()) {
    if (tier.ordinal !== index || tier.lowerBoundMicros !== expectedLower
      || !isCreditCentAligned(tier.lowerBoundMicros)
      || (tier.upperBoundMicros !== null && !isCreditCentAligned(tier.upperBoundMicros))
...
export function cumulativeFeeNumerator(tiersInput: readonly FeeTier[], cumulativeMicros: bigint) {
  if (cumulativeMicros < 0n || !isCreditCentAligned(cumulativeMicros)) throw new Error('FEE_CUMULATIVE_VOLUME_INVALID');
...
export function cumulativeFeeMicros(tiers: readonly FeeTier[], cumulativeMicros: bigint) {
  return roundFractionToCreditCent(cumulativeFeeNumerator(tiers, cumulativeMicros), FEE_DENOMINATOR_BPS);
}

function roundFractionToCreditCent(numerator: bigint, denominator: bigint) {
  if (numerator < 0n || denominator <= 0n) throw new Error('FEE_INTEGER_DOMAIN_INVALID');
  const centDenominator = denominator * KAI_CREDIT_CENT_MICROS;
  return ((numerator + centDenominator / 2n) / centDenominator) * KAI_CREDIT_CENT_MICROS;
}
...
function allocateLargestRemainder(
  ...
  if (!isCreditCentAligned(feeMicros)) throw new Error('FEE_CENT_ALIGNMENT_REQUIRED');
  const feeCents = feeMicros / KAI_CREDIT_CENT_MICROS;
  const working = raw.map((segment) => {
    const weighted = segment.exactFeeNumerator * feeCents;
    return {
      segment,
  ...
  let unallocated = feeCents - working.reduce((sum, item) => sum + item.fee, 0n);
  ...
  return working.map((item) => ({
    ...item.segment, serviceFeeCreditMicros: item.fee * KAI_CREDIT_CENT_MICROS,
  }));
}
...
export function planSettlementFee(
  ...
  if (cumulativeBeforeMicros < 0n || grossCreditMicros <= 0n
    || !isCreditCentAligned(cumulativeBeforeMicros) || !isCreditCentAligned(grossCreditMicros)) {
    throw new Error('FEE_ASSESSMENT_VOLUME_INVALID');
  }
  const tiers = validateFeeTiers(tiersInput);
...
export function planFeeReversal(
  ...
  if (reversedGrossCreditMicros <= 0n || !isCreditCentAligned(reversedGrossCreditMicros)) {
    throw new Error('FEE_REVERSAL_VOLUME_INVALID');
  }
  const originalSegments = [...originalSegmentsInput].sort((left, right) => right.ordinal - left.ordinal);
  if (originalSegments.some((segment) => !isCreditCentAligned(segment.settledCreditMicros)
    || !isCreditCentAligned(segment.serviceFeeCreditMicros)
    || !isCreditCentAligned(segment.reversedCreditMicros)
    || !isCreditCentAligned(segment.reversedFeeCreditMicros))) {
    throw new Error('FEE_REVERSAL_LEGACY_PRECISION_UNSUPPORTED');
  }
  ...
  for (const segment of originalSegments) {
    ...
    const feeAfter = roundFractionToCreditCent(
      segment.serviceFeeCreditMicros * reversedVolumeAfter, segment.settledCreditMicros,
    );
    const feeBefore = roundFractionToCreditCent(
