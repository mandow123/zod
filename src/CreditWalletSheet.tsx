  ...
  return <Modal visible={visible} animationType="slide" transparent onRequestClose={onClose}>
    <View style={styles.backdrop}><View style={styles.sheet}>
      ...
      <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
        ...
        {topupWalletEnabled ? selectedQuickline ? <QuicklineStatusPanel
        ...
        /> : selectedTopup ? <TopupStatusPanel
          topup={selectedTopup}
          querying={querying}
          error={error}
        ...
        /> : <>
          ...
          {loading && (quickline.source === 'staging' ? quicklineTopups.length === 0 : topups.length === 0) ? <ActivityIndicator style={styles.loader} color={colors.primary} /> : null}
          ...
          {quickline.source === 'staging' ? quicklineTopups.length === 0 && !loading ? <View style={styles.empty}><Text style={styles.emptyText}>还没有充值记录</Text></View> : quicklineTopups.map((payment) => <QuicklineRecord key={payment.id} payment={payment} onPress={() => { setSelectedQuickline(payment); setError(null); }} />) : topups.length === 0 && !loading ? <View style={styles.empty}><Text style={styles.emptyText}>还没有充值记录</Text></View> : topups.map((topup) => <Pressable key={topup.id} onPress={() => { setSelectedTopup(topup); setError(null); }} style={styles.record}>
