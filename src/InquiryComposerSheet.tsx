    ...
    <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : 'height'} style={styles.backdrop}>
      <View style={styles.sheet}><View style={styles.handle} />
        ...
        <ScrollView contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator={false}>
          ...
          <Pressable disabled={!canSubmit} onPress={() => void submit()} style={[styles.primary, !canSubmit && styles.disabled]}>{submitting ? <ActivityIndicator color={colors.primary} /> : <Text style={styles.primaryText}>{authenticated ? candidate?.catalog?.serviceMode === 'long_term_machine' ? '提交租赁意向' : '提交询期' : '登录后提交'}</Text>}</Pressable>
