...
type Draft = Omit<CreateResourceInquiryInput, 'candidateId' | 'terms' | 'gpuCount' | 'storageGiB'> & Readonly<{
  gpuCount: string; storageGiB: string;
}>;
...
function createInitialDraft(): Draft {
  return {
    ...initialInquirySchedule(), timeZone: systemTimeZone(), gpuCount: '1', billingMode: 'hourly',
...
export function InquiryComposerSheet({ candidate: initialCandidate, visible, authenticated, onLogin, onClose, onSubmitted }: Readonly<{
  ...
  useEffect(() => {
    ...
    let active = true;
    setCandidate(initialCandidate); setDraft({ ...createInitialDraft(), billingMode: initialCandidate.modes[0] ?? 'hourly' });
    ...
    setLoading(true);
    void Promise.all([loadInquiryCandidate(initialCandidate.candidateId), loadLegalDocuments()])
      .then(([nextCandidate, nextDocuments]) => {
        if (!active) return;
        setCandidate(nextCandidate); setDocuments(nextDocuments);
        setDraft((current) => ({ ...current, billingMode: nextCandidate.modes[0] ?? 'hourly' }));
      })
  ...
  const parsed = useMemo(() => ({
    ...draft, gpuCount: Number(draft.gpuCount), storageGiB: Number(draft.storageGiB),
  }), [draft]);
  const validationError = validateInquiryInput(parsed);
  ...
  const submit = async () => {
    ...
    const input: CreateResourceInquiryInput = {
      ...parsed, candidateId: candidate.candidateId,
      description: parsed.description.trim(), dataRegion: parsed.dataRegion.trim(),
      terms: {
    ...
    <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : 'height'} style={styles.backdrop}>
      <View style={styles.sheet}><View style={styles.handle} />
        ...
        <ScrollView contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator={false}>
          ...
          {candidate ? <View style={styles.candidate}><View><Text style={styles.candidateTitle}>{candidate.model} · {inquiryCardTypeLabel(candidate.cardType)}</Text><Text style={styles.candidateMeta}>{candidate.region} · 待认领供应方</Text></View><Text style={styles.quote}>询期后以卡时报价</Text></View> : null}
          ...
          <View style={styles.twoColumns}><View style={styles.column}><Field label="GPU 数量"><Input value={draft.gpuCount} keyboardType="number-pad" onChangeText={(value) => update('gpuCount', value)} /></Field></View><View style={styles.column}><Field label="存储（GiB）"><Input value={draft.storageGiB} keyboardType="number-pad" onChangeText={(value) => update('storageGiB', value)} /></Field></View></View>
          ...
          <Pressable disabled={!canSubmit} onPress={() => void submit()} style={[styles.primary, !canSubmit && styles.disabled]}>{submitting ? <ActivityIndicator color={colors.surface} /> : <Text style={styles.primaryText}>{authenticated ? '提交询期' : '登录后提交'}</Text>}</Pressable>
