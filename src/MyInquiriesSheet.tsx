...
function InquiryDetail({ inquiry, clarification, setClarification, mutating, error, onClarify, onCancel, onRefresh }: Readonly<{
  ...
  return <ScrollView contentContainerStyle={styles.detail} keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator={false}><View style={styles.detailHero}><Text style={styles.detailTitle}>{inquiryResourceTitle(inquiry)}</Text><Text style={styles.detailRegion}>{inquiry.supplierResource ? inquiry.supplierResource.supplier.legalName : `${inquiry.candidate?.region ?? '区域待确认'} · 待认领供应方`}</Text>{inquiry.supplierResource ? <Text style={styles.unverified}>报价资料导入 · 未经 KAI 验真</Text> : null}<Text style={styles.detailStatus}>{inquiryStatusLabel[inquiry.status]}</Text>{inquiry.statusMessage ? <Text style={styles.description}>{inquiry.statusMessage}</Text> : null}</View>
    <View style={styles.detailCard}>{[
      ['询期编号', inquiry.inquiryNumber], ['使用时段', `${dateTime(inquiry.startsAt, inquiry.timeZone)} 至 ${dateTime(inquiry.endsAt, inquiry.timeZone)}`], ['最晚确认', dateTime(inquiry.confirmBy, inquiry.timeZone)], ['时区', inquiry.timeZone], ['申请数量', inquiryQuantityLabel(inquiry)], ['周期', inquiry.billingMode === 'hourly' ? '按小时' : '包月'], ['最大可接受卡时', `${inquiry.maxCreditAmount} KAI 卡时`], ['接受替代', inquiry.allowSubstitutes ? '是' : '否'], ['环境', requirementLabel(inquiry.requirements.environment)], ['网络', networkLabel(inquiry.requirements.network)], ['存储', `${inquiry.requirements.storageGiB} GiB`], ['数据区域', inquiry.requirements.dataRegion],
...
function inquiryResourceTitle(item: ResourceInquirySummary) {
  if (item.supplierResource) return `${item.supplierResource.specifications.gpu.model} · ${inquiryCardTypeLabel(item.supplierResource.specifications.gpu.formFactor ?? '')}`;
  return item.candidate ? `${item.candidate.model} · ${inquiryCardTypeLabel(item.candidate.cardType)}` : 'GPU 资源待确认';
}
function inquiryQuantityLabel(item: ResourceInquirySummary) {
  if (item.supplierResource) {
    const quantity = item.requestedQuantity ?? item.supplierResource.requestedQuantity;
    const unit = item.supplierResource.quantity.unit === 'server' ? '台' : '个实例';
    const perInstance = item.supplierResource.specifications.gpu.countPerInstance;
    return perInstance === null ? `${quantity} ${unit}` : `${quantity} ${unit} · 共 ${quantity * perInstance} 张 GPU`;
  }
  return item.gpuCount === null ? '待确认' : `${item.gpuCount} 张 GPU`;
}
function inquiryResourceMeta(item: ResourceInquirySummary) {
  return item.supplierResource
    ? `${item.supplierResource.supplier.legalName} · ${inquiryQuantityLabel(item)}`
    : `${item.candidate?.region ?? '区域待确认'} · ${inquiryQuantityLabel(item)}`;
}
