...
export function QixiangTopupPanel({
  ...
  if (selected) {
    ...
    return <View style={styles.section}>
      ...
      {canRecheck ? <Pressable disabled={busy} onPress={() => void recheck()} style={[styles.primary, busy && styles.disabled]}>{busy ? <ActivityIndicator color={colors.surface} /> : <Text style={styles.primaryText}>重新核对</Text>}</Pressable> : null}
  ...
  if (confirming || unresolvedCreate) {
    ...
    return <View style={styles.section}>
      ...
      {error ? <Notice text={error} /> : null}
      <Pressable disabled={busy || !operatorName || amountCents === null} onPress={() => void create()} style={[styles.primary, (busy || !operatorName || amountCents === null) && styles.disabled]}>{busy ? <ActivityIndicator color={colors.surface} /> : <Text style={styles.primaryText}>{unresolvedCreate ? '继续原支付请求' : '确认并前往支付宝'}</Text>}</Pressable>
