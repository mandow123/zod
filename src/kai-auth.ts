...
import {
  ...
  reconcileCommittedKaiOidcSession,
  saveKaiOidcSession,
...
let consentInFlight: Promise<StoredSession> | null = null;
...
function verifiedReasonForPlatformFailure(cause: unknown): VerifiedKaiConnectionReason {
  return platformPendingReason({
    ...
    ...(cause instanceof KaiPlatformResponseError ? { apiCode: 'RESPONSE_INVALID' } : {}),
...
async function platformBootstrap(identity: VerifiedKaiIdentity) {
  try {
    const profile = await apiRequestWithAccessToken<unknown>(
      '/mobile/v1/me', identity.accessToken, identity.idToken, { retry: false },
    );
    const legal = await apiRequestWithAccessToken<unknown>(
      '/mobile/v1/legal', identity.accessToken, identity.idToken, { retry: false },
    );
    return {
      user: decodeKaiPlatformProfile(profile),
      documents: decodeKaiPlatformLegalBootstrap(legal).documents,
    };
...
async function acceptVerifiedKaiConsentsOnce(documents: LegalDocuments) {
  ...
  try {
    ...
    const consent = await apiRequestWithAccessToken<unknown>(
    ...
    decodeKaiPlatformConsent(consent, documents);
    const device = await deviceDescriptor();
    const session = await saveKaiOidcSession({
      accessToken: identity.accessToken,
      refreshToken: identity.refreshToken,
      idToken: identity.idToken,
      scope: identity.scope,
    ...
    return reconcileCommittedKaiOidcSession(session);
  } catch (error) {
    const cause = error instanceof PlatformBootstrapError ? error.cause : error;
    if (cause instanceof ApiError && cause.status === 401) {
      const resolution = await resolvePlatformUnauthorized({
