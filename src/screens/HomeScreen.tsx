...
export function HomeScreen({ snapshot, refreshing, onRefresh, onNavigate, onOpenDemand, onOpenCredits, onOpenSparkDetail }: Props) {
  ...
  return <View style={styles.root}><ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}
    ...
    <Pressable onPress={() => onNavigate('video')} style={styles.videoCta}><View style={styles.videoCtaIcon}><Ionicons name="sparkles" size={20} color="#8BD8FF" /></View><View style={styles.videoCtaCopy}><Text style={styles.videoCtaTitle}>KAI Motion Lab</Text><Text style={styles.videoCtaText}>一句话生成你的 3D KAI 视频</Text></View><Ionicons name="arrow-forward" size={18} color="#8BD8FF" /></Pressable>
...
const styles = StyleSheet.create({
  ...
  shortcuts: { flexDirection: 'row', justifyContent: 'space-between', marginTop: 16 }, shortcut: { width: '23%', alignItems: 'center' }, shortcutIcon: { width: 42, height: 42, borderRadius: 12, alignItems: 'center', justifyContent: 'center', backgroundColor: colors.surface, borderWidth: 1, borderColor: colors.line }, shortcutText: { color: colors.ink, fontSize: 9, fontWeight: '700', marginTop: 7 }, videoCta: { minHeight: 72, marginTop: 20, padding: 13, borderRadius: 14, flexDirection: 'row', alignItems: 'center', backgroundColor: '#07182D' }, videoCtaIcon: { width: 40, height: 40, borderRadius: 12, alignItems: 'center', justifyContent: 'center', backgroundColor: '#123453' }, videoCtaCopy: { flex: 1, marginLeft: 11 }, videoCtaTitle: { color: '#FFFFFF', fontSize: 13, fontWeight: '900' }, videoCtaText: { color: '#B9C9DC', fontSize: 10, marginTop: 4 },
