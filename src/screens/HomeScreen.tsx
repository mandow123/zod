...
const shortcuts = [
  { label: '提供算力', icon: 'hardware-chip-outline' as const },
...
export function HomeScreen({ snapshot, refreshing, onRefresh, onNavigate, onOpenDemand, onOpenCredits, onOpenSparkDetail }: Props) {
  ...
  return <View style={styles.root}><ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}
    ...
    <TopVideoHero eyebrow="实时可用资源" title="找到正在可用的算力" detail="从已审核资源中选择地区、时段与交付能力。">
...
const styles = StyleSheet.create({
  root: { flex: 1, backgroundColor: topVideoColors.canvas }, content: { padding: 16, paddingBottom: 32 },
  search: { minHeight: 44, paddingHorizontal: 13, borderRadius: 14, flexDirection: 'row', alignItems: 'center', gap: 9, backgroundColor: 'rgba(255,255,255,0.93)' }, searchText: { color: topVideoColors.ink, fontSize: 12, fontWeight: '700' },
  accountRow: { minHeight: 76, marginTop: 13, paddingHorizontal: 17, borderRadius: 22, flexDirection: 'row', alignItems: 'center', backgroundColor: topVideoColors.surface, borderWidth: 1, borderColor: topVideoColors.line }, accountItem: { flex: 1 }, accountLabel: { color: topVideoColors.muted, fontSize: 9 }, accountValue: { color: topVideoColors.ink, fontSize: 19, fontWeight: '900', marginTop: 5 }, accountDivider: { width: 1, height: 35, marginHorizontal: 14, backgroundColor: topVideoColors.line },
  ...
  sparkCard: { height: 196, marginTop: 20, borderRadius: 12, overflow: 'hidden', justifyContent: 'flex-end' }, sparkImage: { borderRadius: 12 }, sparkScrim: { position: 'absolute', inset: 0, backgroundColor: 'rgba(8,18,35,0.47)' }, sparkCopy: { padding: 15 }, sparkSupplier: { color: '#D9E8F6', fontSize: 9, fontWeight: '800' }, sparkTitle: { color: '#FFFFFF', fontSize: 24, fontWeight: '900', marginTop: 5 }, sparkMeta: { color: '#FFFFFF', fontSize: 10, marginTop: 5 }, sparkAction: { alignSelf: 'flex-start', minHeight: 38, marginTop: 12, paddingHorizontal: 13, flexDirection: 'row', alignItems: 'center', gap: 7, ...ledgerActionButton }, sparkActionText: { ...ledgerActionText, fontSize: 11, fontWeight: '900' },
