...
export function UnifiedAssetsScreen({ snapshot, mode, refreshing, onRefresh, onLogin, onOpenCredits, onOpenOrder,
  ...
  const [provided, setProvided] = useState(mode === 'provider');

  const load = useCallback(async () => {
    if (!snapshot.authenticated) { setPortfolio(null); setError(null); return; }
    setLoading(true); setError(null);
    try { setPortfolio(await loadAssetPortfolio()); }
    catch (reason) { setPortfolio(null); setError(reason instanceof Error ? reason.message : '资产暂时无法读取。'); }
    finally { setLoading(false); }
  }, [snapshot.authenticated, snapshot.currentSubjectId]);

  useEffect(() => { setPortfolio(null); void load(); }, [load]);
  useEffect(() => setProvided(mode === 'provider'), [mode]);
  ...
  return <View style={styles.root}><ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}
    ...
    {!snapshot.authenticated ? <Empty icon="lock-closed-outline" title="登录后查看资产" text="已购算力、设备和供应资产都会保留在同一账号。" action="安全登录" onPress={onLogin} />
      ...
      : portfolio ? <>
        ...
        {provided ? <>
          <Text style={styles.sectionTitle}>供应状态</Text><View style={styles.statuses}>{[
          ...
          {portfolio.groups.providedCompute.map((item) => <AssetRow key={item.id} title={item.title} meta={`${item.region} · ${item.statusLabel}`} action={item.actions[0]} onPress={() => item.actions[0] ? openAction(item, item.actions[0]) : onOpenProviderAssets(item.resourceId)} />)}
          ...
          {!portfolio.groups.purchasedCompute.length && !portfolio.groups.purchasedDeviceOrders.length && !portfolio.groups.ownedDevices.length ? <Empty icon="cube-outline" title="还没有购买资产" text="市场只展示服务端确认可购买的资源。" action="去市场看看" onPress={onOpenMarket} /> : null}
        </>}
      </> : null}
  </ScrollView></View>;
}
