...
function fixture() {
  return {
    ...
    ios: {
      ...
      associatedDomains: ['applinks:cloudpay.kai.com'], config: { usesNonExemptEncryption: false },
    ...
    extra: {
      ...
      accountDeletionUrl: 'https://cloudpay.kai.com/account/delete', kaiAuthUniversalLinksEnabled: true,
    },
  };
}
...
test('release policy rejects commerce, identity, API, legal, push, icon, and privacy regressions', () => {
  const mutations = [
    ...
    (config) => { config.extra.kaiAuthUniversalLinksEnabled = false; },
  ];
