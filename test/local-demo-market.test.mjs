...
test('本地验收挂牌保留不可购买事实且没有真实交易入口', () => {
  assert.match(api, /purchasable: listing\.demo\.simulatedAudit \? false : listing\.demo\.purchasable/u);
  assert.match(api, /blockedReason: listing\.demo\.simulatedAudit \|\| !listing\.demo\.purchasable \? '本地验收资源，不进入真实交易' : null/u);
  ...
  assert.match(market, /platformDemoOnly \? '本地验收目录，不进入真实交易' : '即时开通，按卡时结算'/u);
  const demoCard = market.slice(market.indexOf('function LocalDemoMarketRow'), market.indexOf('function MarketRow'));
  for (const copy of ['本地验收资源', '非生产库存', '演示容量', '仅供本机验收']) assert.match(demoCard, new RegExp(copy, 'u'));
  ...
  assert.equal(listingAvailability(ready, { purchasable: false, blockedReason: '本地验收资源，不进入真实交易' }, true).allowed, false);
