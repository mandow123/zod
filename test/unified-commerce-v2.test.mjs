...
test('device purchase uses only the authoritative product and order endpoints', async () => {
  const [api, order, app] = await Promise.all([
    source('../src/api.ts'), source('../src/DeviceOrderSheet.tsx'), source('../App.tsx'),
  ]);
  ...
  assert.match(api, /02672000-0000-4000-8000-000000000200|device-products/u);
  assert.match(order, /product\.purchasable/u);
...
test('production Spark cards come from the authoritative device catalog without demo duplication', async () => {
  const [api, home, market] = await Promise.all([
    source('../src/api.ts'), source('../src/screens/HomeScreen.tsx'), source('../src/screens/MarketScreen.tsx'),
  ]);
  assert.match(api, /apiRequest<DeviceProductsResponse>\('\/mobile\/v1\/device-products', \{ auth: 'optional'/u);
  ...
  assert.match(market, /product\.purchasable/u);
});
...
test('provider asset totals never estimate settled earnings from gross closed orders', async () => {
  ...
  assert.match(assets, /snapshot\.creditBalance\.redeemableSupplierEarnings/u);
  assert.match(assets, /snapshot\.creditBalance\.supplierReceivable/u);
...
test('unified assets shows backend evidence for device states and payout balances', async () => {
  ...
  assert.match(assets, /snapshot\.deviceAssets/u);
  assert.match(assets, /snapshot\.deviceOrders/u);
  assert.match(assets, /snapshot\.payouts/u);
  assert.match(assets, /payoutFrozen/u);
...
test('device checkout uses a normal address book and keeps references internal', async () => {
  const [api, order] = await Promise.all([source('../src/api.ts'), source('../src/DeviceOrderSheet.tsx')]);
  assert.match(api, /GET|loadShippingAddresses/u);
  assert.match(api, /\/mobile\/v1\/shipping-addresses/u);
  assert.match(api, /createShippingAddress/u);
  assert.match(api, /deleteShippingAddress/u);
